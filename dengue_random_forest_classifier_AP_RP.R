...
###Classifying patients into AP vs RP groups
...
df <- read.csv("C:/dengue/DENV_filtered_imputed_dataset.csv", header = TRUE)

# Filter for AP and RP groups only
df <- df %>% filter(Group %in% c("AP", "RP"))

# Rename columns for consistency with the template
colnames(df) <- c("Genes", "patient_id", "Group", "Run", "Classification", "LFQ")

# Use Group as the class variable
df$class <- df$Group
...
if(sum(patient_counts$n_observations > 1) > 0) {
  cat("The following patients have multiple observations:\n")
  print(patient_counts %>% filter(n_observations > 1))
...
X_train <- train_data %>% select(-patient_id, -class) %>% as.matrix()
X_test <- test_data %>% select(-patient_id, -class) %>% as.matrix()
...
###data-points in AP and RP may be unequal
...
cat("=== SHAP VALUE ANALYSIS (AP vs RP) ===\n")
...
if(nrow(shap_importance_df) > 0) {
  ...
  shap_plot <- ggplot(top_shap_features, aes(x = reorder(Feature, MeanAbsSHAP), y = MeanAbsSHAP)) +
    ...
    coord_flip() +
    labs(
      title = "Top 20 SHAP Feature Importances (AP vs RP)",
      x = "Gene",
      y = "Mean |SHAP|"
    ) +
...
p2 <- ggplot(prob_df, aes(x = Probability, fill = True_Class)) +
  ...
  labs(title = "Predicted Probability Distribution",
       ...
       fill = "True Class") +
  theme_minimal(base_size = 12) +
  scale_fill_manual(values = c("AP" = "#E64B35", "RP" = "#4DBBD5")) +
...
p3 <- ggplot(prob_summary, aes(x = reorder(Patient_ID, Mean_Prob),
  ...
  labs(title = "Per-Patient Prediction Confidence",
       subtitle = "Mean probability ± SD",
       x = "Patient ID", y = "Predicted Probability",
       color = "True Class") +
  theme_minimal(base_size = 10) +
  scale_color_manual(values = c("AP" = "#E64B35", "RP" = "#4DBBD5")) +
